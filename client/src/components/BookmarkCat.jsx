  ...
  theList.map((url) =>
    {
      if(url.category === item.props) {
        ...
        arr.push(<li  key={url._id} ><a href={url.url} target='_blank'className='trydeed' >{url.description}</a><button value={url._id} className='deleteBut' onClick={delBM}>🗑️</button></li>);
        }
      }      
    )
    return(arr)
