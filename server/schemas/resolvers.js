...
const resolvers = {
  Query: {
    getAUsersBookmarks: async (parent, { username }) => {
      const allBooks = await Bookmark.find({ username: username }).sort(
        "category",
      );
      return allBooks;
    },
  ...
  Mutation: {
    ...
    addBookmark: async (parent, { _id, url, description, category }) => {
      console.log(
        "🚀 ~ addBookmark: ~ username,url, description, category:",
        _id,
        url,
        description,
        category,
      ...
      const username = _id;
      const newUrl = await Bookmark.create({
        description,
        url,
        category,
        username,
    ...
    updateBookmark: async (parent, { _id, url, description, category }) => {
      const updBookmark = await Bookmark.findByIdAndUpdate(
        { _id: _id },
